...
def main(args):
    ...
    for ep in range(args.epochs):
        ...
        if (ep + 1) % args.eval_every == 0 or ep == args.epochs - 1:
            ...
            cacc = float((np.argmax(fld, 1) == np.argmax(lte.numpy(), -1)).mean())
            print(f"  ep{ep+1:3d} loss={tot/M:.4f}  cell-acc={cacc:.3f}  board-acc={acc.mean():.3f}  {time.time()-t0:.0f}s", flush=True)
    model.eval()
    fld = gd_sample(model, cond_te, args.eta, args.steps, clamp_feats=clamp_te).cpu().numpy()
