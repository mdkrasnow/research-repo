"""
Ladder rung 13: PRIOR-BUDGET test — how weak can the operator prior be and still reach near-oracle?

Rung 12: GEN_SKEW (M=exp(theta·skew(A)) = rotation hardcoded) reached ORACLE; GEN_FREE (general exp(A)
+ det/ortho reg) reached ~69%. The skew result names the symmetry class. Rung 13 asks: can we WEAKEN
the prior from "it is a rotation" to "it is a smooth, volume-preserving, well-conditioned group action"
and keep near-oracle, with rotation EMERGING rather than hardcoded?

Arms: BASE / ORACLE / GEN_SKEW (strong-prior near-oracle ref) / GEN_FREE (weak: det+weak-ortho,
rung-12 ~69% baseline) / GEN_FREE_STABLE (new: det + well-conditioned cond->1, NO skew param, NO direct
isometry term — stability only). Same frozen anchor, clean latent (enc->z), no live-field closure, no
held-out targets, no mining.

Caveat (logged): in 2D latent, "volume-preserving + well-conditioned" ≈ "rotation" (det=1 + cond=1 ->
orthogonal -> rotation). So 2D partly conflates the priors; a definitive prior-budget test wants a
HIGHER-DIM latent where stable-group-action strictly contains rotation (candidate rung 14).

Original rung-12 header: GROUP-STRUCTURED generator — close the operator-PRECISION gap.
...
ALPHA_AUG=1.0; LAM_MOVE=0.5; LAM_DET=1.0; LAM_ORTHO=0.3; LAM_COND=1.0; W_WD=1e-4; DEC_SEED=1234; TARGET_DEG=60
ARMS=["BASE","ORACLE","GEN_SKEW","GEN_FREE","GEN_FREE_STABLE"]
...
class LatentOp(nn.Module):
    ...
    def M(self):
        ...
        return torch.matrix_exp(self.A)                      # free / stable
    def op_params(self):
        return [self.M_param] if self.kind=="matrix" else ([self.A,self.theta] if self.kind=="skew" else [self.A])
    def reg(self):
        I=torch.eye(K_LAT,device=DEV)
        if self.kind=="free":      # weak prior: volume (det~1) + weak isometry (rung-12 FREE)
            M=self.M(); det=torch.det(M); ortho=((M.T@M-I)**2).sum()
            return LAM_DET*(torch.log(det.abs()+1e-8))**2 + LAM_ORTHO*ortho
        if self.kind=="stable":    # "stable group action": volume (det~1) + well-conditioned (cond->1).
            M=self.M(); det=torch.det(M)                      # NO skew param, NO direct isometry term.
            sv=torch.linalg.svdvals(M)                        # rotation must EMERGE, not be hardcoded.
...
def run_arm(arm, seed):
    ...
    if arm=="GEN_SKEW": T=anchor_train_op(seed,"skew"); return train_field(seed,T=T), T
    if arm=="GEN_FREE": T=anchor_train_op(seed,"free"); return train_field(seed,T=T), T
    if arm=="GEN_FREE_STABLE": T=anchor_train_op(seed,"stable"); return train_field(seed,T=T), T
...
def main():
    ...
    orc=results["arms"]["ORACLE"]["recall_arc"]
    print(f"\nideal recall_arc={45/360:.3f}. PRIOR-BUDGET: does GEN_FREE_STABLE (no skew) approach GEN_SKEW/ORACLE?")
    for a in ("GEN_SKEW","GEN_FREE","GEN_FREE_STABLE"):
        ag=results["arms"][a]
        pct=100*ag["recall_arc"]/orc if orc>0 else 0
        print(f"  {a}: recall={ag['recall_arc']:.3f} ({pct:.0f}% ORACLE) angle_err_from_45={ag.get('angle_err45','-')} shift_std={ag.get('T_shift_std','-')}")
