"""
Ladder rung 8: MINIMAL identity-exclusion prior (continuous manifold).

Synthesis after rungs 5-7: the learned operator always collapses to the IDENTITY (a valid symmetry
satisfying every prior-free constraint); soft anti-identity penalties fail. rung-8 tests the minimal
prior that excludes identity WITHOUT asserting a group: hard-project M onto the sphere ||M-I||=c after
each step (direction free). Drop soft move+cyclic. closure-aug then picks the ON-MANIFOLD direction
among fixed-size transforms = the symmetry. If ORACLE_LATENT now recovers a rotation + fills the arc,
the answer to "how much prior is needed" = "just enough to exclude identity." If even this fails,
discovery is harder than identity-exclusion. (c=1.4 ~ 60deg rotation; >gap so it fills the arc.)

Original rung-7 header: CONTINUOUS manifold so continuity forces the symmetry.
...
C_IDEXCL=1.4   # rung-8: hard-fix ||M-I||=c (excludes identity; c~1.08->45deg, 1.4->~60deg). Direction FREE.

def project_M(T):
    # project M onto the sphere ||M-I||_F = C_IDEXCL around the identity (identity-exclusion prior)
    with torch.no_grad():
        I=torch.eye(K_LAT,device=DEV); d=T.M-I; n=d.norm()
        if float(n)>1e-8: T.M.copy_(I + C_IDEXCL*d/n)

def disc_recipe(loss,f,T,x):
    # rung-8: identity excluded structurally by project_M -> drop soft move+cyclic. Only closure-aug:
    # M is held off identity; closure picks the ON-MANIFOLD direction (= the symmetry) among all
    # transforms of fixed size. No assertion that M is a rotation.
    loss=loss+W_AUG*eqm_loss(f,T(x))
    return loss
...
def train(arm, seed, steps, ae_steps, joint_mode):
    ...
    for step in range(steps):
        ...
        if arm in ("ORACLE_LATENT","DISC_JOINT") and hasattr(T,"M"): project_M(T)   # identity-exclusion
    if arm=="DISC_JOINT":
