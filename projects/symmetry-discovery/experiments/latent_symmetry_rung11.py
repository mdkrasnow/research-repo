"""
Ladder rung 11: orbit augmentation — close the COVERAGE gap.

Rung 10: frozen anchor + single latent matrix M recovered a coherent rotation (M_angle ~-39deg) and
filled PART of the held-out arc (recall 0.025 vs ORACLE 0.068). Gap = one generator covers only a slice
of the arc; ORACLE used the full random-rotation group. Fix (one change): augment EqM with the OPERATOR
ORBIT {M^1..M^{P-1}} from the SAME single discovered generator -> approximate the group -> cover the
full arc. If FROZEN_LATENT_ORBIT approaches ORACLE, discovery is closed: coverage was the last gap.

Arms: BASE / ORACLE / FROZEN_LATENT_SINGLE (rung-10 single aug) / FROZEN_LATENT_ORBIT (orbit aug,
clean latent) / FROZEN_LATENT_DISC_ORBIT (orbit aug, fully-unsupervised learned dec).

Original rung-10 header: frozen anchor + COHERENT operator (single latent matrix).
...
ARMS=["BASE","ORACLE","FROZEN_LATENT_SINGLE","FROZEN_LATENT_ORBIT","FROZEN_LATENT_DISC_ORBIT"]
...
P_ORBIT=8   # apply random power M^k, k in 1..P_ORBIT-1 -> approximate the GROUP from one generator

@torch.no_grad()
def orbit_aug_input(T, x, k):
    Mk=torch.linalg.matrix_power(T.M, k)
    return T.decode(T.enc(x)@Mk.T)

def train_field(seed, T=None, oracle=False, orbit=False):
    torch.manual_seed(seed); f=mlp([D+1,256,256,D]).to(DEV); opt=torch.optim.Adam(f.parameters(),lr=2e-3)
    for step in range(EQM_STEPS):
        ...
        elif T is not None and orbit:
            k=int(step%(P_ORBIT-1))+1                       # cycle through orbit powers M^1..M^{P-1}
            loss=loss+ALPHA_AUG*eqm_loss(f, orbit_aug_input(T,x,k))
        elif T is not None:
...
def run_arm(arm, seed):
    ...
    if arm=="FROZEN_LATENT_SINGLE": T=anchor_train_latent(seed, learned_dec=False); return train_field(seed,T=T), T
    if arm=="FROZEN_LATENT_ORBIT":  T=anchor_train_latent(seed, learned_dec=False); return train_field(seed,T=T,orbit=True), T
    if arm=="FROZEN_LATENT_DISC_ORBIT": T=anchor_train_latent(seed, learned_dec=True); return train_field(seed,T=T,orbit=True), T
