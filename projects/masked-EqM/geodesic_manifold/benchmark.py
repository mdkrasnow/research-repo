...
def main(argv=None):
  ...
  result={"single_seed_preliminary":True,"checkpoints":{v:records[v].__dict__ for v in records},"calibration":{v:calibrations[v].__dict__ for v in calibrations},"linear_metric_infeasibility":metric_fallback,"unavailable_secondary_controls":unavailable,"rows":table,"direct_dot_relative_improvement":{"mean":float((dot.mean()-direct.mean())/max(dot.mean(),1e-12)),"ci95":np.quantile(boot,[.025,.975]).tolist(),"definition":"(mean(A_dot)-mean(A_direct))/mean(A_dot); pair-bootstrap CI"},"artifacts":{"paired_tradeoff_csv":"paired_tradeoff.csv","paired_tradeoff_plot":"paired_tradeoff.png"},"warning":"No seed-level or preregistered pass/fail inference: only one matched checkpoint per method; any exponential-metric result is secondary. none_gradient_norm is a labelled gradient-field control, not an energy-value comparison."}
