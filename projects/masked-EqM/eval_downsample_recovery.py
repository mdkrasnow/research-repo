...
"""
Downsample-recovery eval for structured start-state EqM checkpoints
(bilinear downsample/upsample corruption family, added 2026-07-14
alongside fourier to test whether the masking result generalizes to a
spatial-resampling corruption).

Given a checkpoint, corrupts held-out real images via downsample/upsample
(same transport.corruption.downsample_corrupt used in training), starts
the GD/NAG sampler from that corrupted latent instead of pure noise, and
measures full-image recovery MSE (and LPIPS if available) vs the
original. Whole-image corruption (no held-out visible region), so no
...
training corruption family (pass --factor-grid to sweep severities).
"""
import argparse
import json
import os
...
def gd_recover(model_fn, z0, y, num_sampling_steps, stepsize, sampler, mu):
    ...
    No hard-constrain option here -- downsample corrupts the whole image,
...
def main(args):
    ...
    factors = [float(f) for f in args.factor_grid.split(",")] if args.factor_grid else [args.factor]
    ...
    with torch.no_grad():
        for factor in factors:
            ...
            for start in range(0, len(indices), args.batch_size):
                ...
                z0 = downsample_corrupt(x1, factor)
            ...
            print(f"factor={factor} mean_mse={per_factor[str(factor)]['mean_mse']:.5f} "
                  f"mean_lpips={per_factor[str(factor)]['mean_lpips']}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--factor", type=float, default=4.0,
                         help="single downsample factor for the recovery test (ignored if --factor-grid set)")
    parser.add_argument("--factor-grid", type=str, default=None,
                         help="comma-separated list of factors for a held-out severity grid, e.g. '2,4,8,16'")
    parser.add_argument("--vae-roundtrip-oracle", action="store_true",
                         help="positive control: skip downsample/model, just measure VAE encode->decode floor")
    ...
    parser.add_argument("--out", type=str, default="eval_results/downsample_recovery.json")
