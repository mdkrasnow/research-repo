...
def main(args):
    ...
    # Run identity, resolved BEFORE anything is written to disk: the experiment
    # directory name is derived from it (see resolve_experiment_dir), so the
    # name no longer depends on how many sibling entries happen to exist.
    run_spec, spec_provenance = resolve_run_spec(args)
    telemetry_root = telemetry_root_for(args.results_dir)

    # Setup an experiment folder:
    if rank == 0:
        os.makedirs(args.results_dir, exist_ok=True)  # Make results folder (holds all experiment subfolders)
        experiment_dir = resolve_experiment_dir(args.results_dir, args, run_spec)
        experiment_name = os.path.basename(experiment_dir)
        checkpoint_dir = f"{experiment_dir}/checkpoints"  # Stores saved model checkpoints
        os.makedirs(checkpoint_dir, exist_ok=True)
        logger = create_logger(experiment_dir)
        logger.info(f"Experiment directory created at {experiment_dir}")
        logger.info(f"Run identity: provenance={spec_provenance} "
                    f"run_uid={getattr(run_spec, 'run_uid', None)} "
                    f"telemetry_root={telemetry_root}")
        if spec_provenance != "stated":
            logger.warning(
                "[telemetry] EQM_RUN_SPEC absent or unusable -- run identity was "
                f"INFERRED from argv (provenance={spec_provenance}). Analyses must "
                "not treat this run as launcher-registered.")
    ...
    # The step count this execution is SUPPOSED to reach.  Recorded in START and
    # compared against last_step in END, which is what makes truncation a
    # machine-checkable fact instead of something an analyst has to notice.
    planned_steps = (max_train_steps if max_train_steps is not None
                     else args.epochs * len(loader))
    nonfinite_grad_events = 0
    telemetry_run = open_run_recorder(
        rank, telemetry_root, run_spec, planned_steps=planned_steps,
        logger=logger,
        extra_start={
            "spec_provenance": spec_provenance,
            "experiment_dir": experiment_dir if rank == 0 else None,
    ...
    # The recorder is a context manager for a reason: the ONLY way this loop can
    # exit without a terminal END record is SIGKILL/node death (sealed outside the
    # process).  Normal return, exception, and SIGTERM/SIGUSR1/SIGINT all unwind
    # through __exit__.  Note it swallows the preemption signal by design.
    with telemetry_run:
        for epoch in range(resume_epoch, args.epochs):
            ...
            for x, y in loader:
                ...
                if grad_norm is not None and rank == 0 and args.grad_log_every > 0:
                    ...
                    if args.grad_log_every > 0 and next_step % args.grad_log_every == 0:
                            ...
                            if args.btm_eval_every > 0 and \
                                ...
                                try:
                                    if btm_cfg.mode == "btm_vector":
                                        btm_eval_stats = btm_eval_target_match_vector(
                                            model.module, btm_cfg, transport,
                                            _probe_x1, probe_y)
                                    else:
                                        btm_eval_stats = btm_eval_target_match(
                                            model.module, btm_cfg, transport,
                                            _probe_x1, probe_y)
                                    record.update(btm_eval_stats)
                                finally:
                                    model.module.train(was_training)
                                # Expensive + periodic -> EVAL, not PROGRESS: pooling
                                # a cadence-N evaluation with a cadence-1 stream is
                                # how a per-step median silently becomes a mixture.
                                telemetry_run.evaluation(
                                    train_steps + 1, kind="target_match",
                                    btm_mode=btm_cfg.mode,
                                    **{k: v for k, v in btm_eval_stats.items()
                                       if k != "step"})
                        ...
                        # Legacy stream (dual-write during migration: runs in flight
                        # -- SLURM 39090540/41/42/739 -- parse this exact format).
                        grad_metrics_file.write(json.dumps(record, sort_keys=True) + "\n")
                        # New stream.  kind="grad" == the INPUT to optimizer step
                        # `next_step`; the kind="probe" record emitted after
                        # opt.step() below carries the SAME step number and measures
                        # that step's EFFECT.  See train_utils' convention note.
                        telemetry_run.progress(
                            next_step, kind="grad",
                            **{k: v for k, v in record.items() if k != "step"})
                        if is_nonfinite(record["grad_norm"]) or is_nonfinite(record["loss"]):
                            nonfinite_grad_events += 1
                            telemetry_run.notice(
                                f"nonfinite gradient/loss at step {next_step}: "
                                f"grad_norm={record['grad_norm']} loss={record['loss']}",
                                level="warn", step=next_step,
                                nonfinite_grad_events=nonfinite_grad_events)
                opt.step()
                ...
                if grad_norm is not None and rank == 0 and args.grad_log_every > 0 \
                    ...
                    # Legacy stream (dual-write; structurally different from the
                    # gradient record above and, in the legacy file, indistinguishable
                    # from it -- which is the defect the new stream's `kind` fixes).
                    grad_metrics_file.write(json.dumps(checklist_record, sort_keys=True) + "\n")
                    # New stream: same step number as the kind="grad" record above,
                    # because this measures the EFFECT of that same optimizer step.
                    telemetry_run.progress(
                        train_steps + 1, kind="probe",
                        **{k: v for k, v in checklist_record.items() if k != "step"})
                ...
                if train_steps % args.log_every == 0:
                    ...
                    # steps_per_sec / peak memory / RSS previously existed only in the
                    # text log (or not at all), so no analysis could read them back.
                    telemetry_run.progress(
                        train_steps, kind="throughput",
                        avg_loss=avg_loss, steps_per_sec=steps_per_sec,
                        peak_gpu_memory_bytes=peak_gpu_memory_bytes(),
                        host_rss_bytes=host_rss_bytes(),
                        nonfinite_grad_events=nonfinite_grad_events,
                        epoch=epoch,
                    )
                    telemetry_run.heartbeat(train_steps)
                ...
                if train_steps % args.ckpt_every == 0 and train_steps > 0:
                    if rank == 0:
                        ...
                        telemetry_run.artifact(checkpoint_path, step=train_steps,
                                               kind="checkpoint", epoch=epoch + 1)
                    dist.barrier()

                telemetry_run.set_last_step(train_steps)
            ...
            if rank == 0 and (args.save_epochs is None or epoch + 1 in args.save_epochs):
                ...
                telemetry_run.artifact(epoch_checkpoint_path, step=train_steps,
                                       kind="epoch_checkpoint", epoch=epoch + 1)
            dist.barrier()
        telemetry_run.set_last_step(train_steps)
...
def main_forward_backwards_direct(args):
    ...
    # Identity first; the directory name is derived from it (same fix as main()).
    run_spec, spec_provenance = resolve_run_spec(args)
    telemetry_root = telemetry_root_for(args.results_dir)
    ...
    telemetry_run = open_run_recorder(
        ...
        extra_start={
            ...
            "steps_per_epoch": len(loader),
            "local_batch_size": local_batch_size,
            "training_path": "forward_backwards_direct",
        },
    )
    # Terminal-record guarantee, same ladder as main(): see telemetry/lifecycle.py.
    with telemetry_run:
        for epoch in range(resume_epoch, args.epochs):
            sampler.set_epoch(epoch)
            logger.info(f"Beginning epoch {epoch}...")
            ...
            for x, y in loader:
                ...
                # Snapshot the per-step keys BEFORE any audit merges into the same
                # dict: the audits run on a different (slower) cadence, and pooling
                # the two populations on one stream is the defect being removed.
                step_diagnostics = dict(diagnostics)
                ...
                if (
                    ...
                    telemetry_run.evaluation(
                        train_steps + 1, kind="exact_field_audit",
                        **{k: v for k, v in audit.items() if k != "step"})
                    ...
                    telemetry_run.evaluation(
                        train_steps + 1, kind="exact_gradient_audit",
                        **{k: v for k, v in grad_audit.items() if k != "step"})

                if rank == 0 and fb_metrics_file is not None:
                    # Legacy stream, unchanged (dual-write during migration).
                    record = {"step": train_steps + 1, **diagnostics}
                    fb_metrics_file.write(json.dumps(record, sort_keys=True) + "\n")
                # New stream.  Same convention as main(): step train_steps+1 is the
                # optimizer step just taken, kind="fb_step" is its per-step record.
                telemetry_run.progress(
                    train_steps + 1, kind="fb_step",
                    **{k: v for k, v in step_diagnostics.items() if k != "step"})

                loss_value = loss.item()
                if is_nonfinite(loss_value):
                    nonfinite_grad_events += 1
                    telemetry_run.notice(
                        f"nonfinite loss at step {train_steps + 1}: {loss_value}",
                        level="warn", step=train_steps + 1,
                        nonfinite_grad_events=nonfinite_grad_events)
                running_loss += loss_value
                log_steps += 1
                train_steps += 1
