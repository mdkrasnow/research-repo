...
def main(args):
    ...
    if rank == 0:
        ...
        if args.wandb:  # only require ENTITY/PROJECT env when wandb is actually used
    ...
    log_steps = 0
    running_loss = 0
    # ANM diagnostics (accumulated only when --anm-steps > 0)
    running_anm = {"anm_disp_ratio": 0.0, "anm_res_before": 0.0, "anm_res_after": 0.0}
    start_time = time()
    ...
    train_start = time()          # wall-clock anchor for --max-minutes cap
    stop_training = False
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            anm = dict(steps=args.anm_steps, step_size=args.anm_step_size,
                       eps_ball=args.anm_eps_ball) if args.anm_steps > 0 else None
            loss_dict = transport.training_losses(model, x, model_kwargs, anm=anm)
            ...
            if anm is not None:
                for k in running_anm:
                    if k in loss_dict:
                        running_anm[k] += float(loss_dict[k])
            log_steps += 1
            train_steps += 1
            if train_steps % args.log_every == 0:
                ...
                wandb_payload = { "train loss": avg_loss, "train steps/sec": steps_per_sec }
                if anm is not None:
                    anm_avg = {k: running_anm[k] / log_steps for k in running_anm}
                    logger.info(
                        f"(step={train_steps:07d}) ANM disp_ratio: {anm_avg['anm_disp_ratio']:.4f} "
                        f"(||x0_adv-x0||/||x0||; ~0 == no-op), "
                        f"residual {anm_avg['anm_res_before']:.4f} -> {anm_avg['anm_res_after']:.4f}"
                    )
                    wandb_payload.update(anm_avg)
                    for k in running_anm:
                        running_anm[k] = 0.0
                if args.wandb:
...
if __name__ == "__main__":
    ...
    # Adversarial Negative Mining (ANM): PGD loss-ascent on the EqM endpoint.
    parser.add_argument("--anm-steps", type=int, default=0,
                        help="ANM: # PGD ascent steps on the negative endpoint (0 disables ANM)")
    parser.add_argument("--anm-step-size", type=float, default=0.15,
                        help="ANM: per-step displacement as a FRACTION of per-sample ||x0|| "
                             "(dimension-free; retune via the logged anm_disp_ratio)")
    parser.add_argument("--anm-eps-ball", type=float, default=0.5,
                        help="ANM: L2 trust-region radius as a fraction of ||x0|| (0 disables projection)")
